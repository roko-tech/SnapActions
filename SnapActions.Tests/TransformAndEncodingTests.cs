...
public class TransformAndEncodingTests
{
    ...
    [Fact]
    public void Case_WordTransforms_UseInvariantCulture()
    {
        // Regression: camel/pascal/snake/kebab used CurrentCulture ToLower/ToUpper, which mangles
        // I/i on Turkish-family locales (dotted/dotless-I) — snake_case("HELLO_INDIA") came out as
        // "hello_ındıa". They now use the *Invariant overloads like ToTitleCase does.
        var prev = System.Threading.Thread.CurrentThread.CurrentCulture;
        try
        {
            System.Threading.Thread.CurrentThread.CurrentCulture = new System.Globalization.CultureInfo("tr-TR");
            Assert.Equal("hello_india", Run(ActionCategory.Transform, "case_snake", "HELLO_INDIA"));
            Assert.Equal("hello-india", Run(ActionCategory.Transform, "case_kebab", "HELLO INDIA"));
            Assert.Equal("helloIndia", Run(ActionCategory.Transform, "case_camel", "hello india"));
            Assert.Equal("HelloIndia", Run(ActionCategory.Transform, "case_pascal", "hello india"));
        }
        finally { System.Threading.Thread.CurrentThread.CurrentCulture = prev; }
    }
