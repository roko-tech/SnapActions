...
public class MathEvaluatorTests
{
    ...
    [Fact]
    public void Evaluate_DeepPowerChain_ThrowsInsteadOfCrashing()
    {
        // Regression: ParsePower recursed on right-associative '^' WITHOUT the depth guard that
        // ParseExpression has, so a long "2^2^2^...^2" selection blew the managed stack with an
        // *uncatchable* StackOverflowException that killed the whole process. The guard now turns
        // it into a catchable FormatException well before the stack is exhausted.
        var deepPowerChain = "2" + string.Concat(System.Linq.Enumerable.Repeat("^2", 100));
        Assert.Throws<System.FormatException>(() => MathEvaluator.Evaluate(deepPowerChain));
    }
